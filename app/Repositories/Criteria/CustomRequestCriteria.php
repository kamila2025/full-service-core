...
class CustomRequestCriteria extends BaseRequestCriteria
{
        ...
        if ($search && is_array($fieldsSearchable) && count($fieldsSearchable)) {
            ...
            $model = $model->where(function ($query) use ($fields, $search, $searchData, $isFirstField, $modelForceAndWhere) {
                ...
                foreach ($fields as $field => $condition) {
                    ...
                    if ($condition === 'between' || $condition === 'timestamp') {
                        $value = explode(',', $value);
                        if (count($value) < 2) {
                            $value = null;
                        }
                        ...
                        if (!is_null($value)) {
                            if (!is_null($relation)) {
                                $query->whereHas($relation, function ($query) use ($field, $condition, $value) {
                                    ...
                                    } elseif ($condition === 'timestamp') {
                                        $query->whereBetween($field, [Carbon::createFromTimestamp($value[0]), Carbon::createFromTimestamp($value[1])]);
                                    } else {
                                ...
                                if ($condition === 'in') {
                                    ...
                                    $query->whereBetween($modelTableName . '.' . $field, [Carbon::createFromTimestamp($value[0]), Carbon::createFromTimestamp($value[1])]);
                                } else {
